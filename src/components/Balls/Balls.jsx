  ...
  return (
    <>
          ...
          list.map((ball, i) => (
            <Col sm='3' key={ball.id}>
              <Card>
                ...
                <CardBody data-testid="card-ball">
                  <CardTitle>{ball.name}</CardTitle>
                  {ball.owner !== profile.id ? (
                    <>
                      <Badge data-testid="ball-missing" color='danger'>Não encontrada</Badge>
                      <Button data-testid="ball-button" size='sm' color='warning' onClick={() => validateBall(ball)}>
                        encontrei
                      </Button>
                    </>
                  ) : (
      ...
      <Modal isOpen={modal} toggle={toggle}>
        <ModalBody data-testid="ball-confirm">
          <FormGroup>
        ...
        <ModalFooter>
        <Button data-testid="ball-success" color='success' onClick={() => updateList(currentBall?.id)}>
            Validar
          </Button>
